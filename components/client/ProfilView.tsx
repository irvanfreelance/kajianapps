...
function MenuBtn({ icon: Icon, label, onClick, danger }: { icon: any, label: string, onClick?: () => void, danger?: boolean }) {
  return (
    <button onClick={onClick} style={{ display: "flex", alignItems: "center", gap: 14, width: "100%", padding: "16px 20px", background: "#18181F", borderRadius: 16, border: `1px solid rgba(212,175,55,0.08)`, cursor: "pointer" }}>
      <Icon size={20} color={danger ? "#F87171" : GOLD} />
      <span style={{ flex: 1, textAlign: "left", fontSize: 14, fontWeight: 600, color: danger ? "#F87171" : "#fff" }}>{label}</span>
      <ChevronRight size={18} color="rgba(212,175,55,0.3)" />
    </button>
  );
}
...
export function ProfilView() {
  ...
  return (
    <div style={{ paddingBottom: 20, background: DARK, minHeight: "100vh" }}>
      {/* Header */}
      <div style={{ background: "linear-gradient(160deg, #1C1A0F, #141420)", padding: "48px 20px 64px", display: "flex", flexDirection: "column", alignItems: "center", position: "relative", overflow: "hidden" }}>
        <div style={{ position: "absolute", top: -40, right: -40, width: 180, height: 180, borderRadius: "50%", background: "radial-gradient(circle, rgba(212,175,55,0.1), transparent 70%)" }} />
        <div style={{ width: 90, height: 90, borderRadius: "50%", background: "rgba(212,175,55,0.1)", padding: 4, border: `2px solid rgba(212,175,55,0.3)`, position: "relative", overflow: "hidden" }}>
          <div style={{ width: "100%", height: "100%", borderRadius: "50%", background: "linear-gradient(135deg, rgba(212,175,55,0.2), rgba(212,175,55,0.05))", display: "flex", alignItems: "center", justifyContent: "center", position: "relative", overflow: "hidden" }}>
            {session?.user?.image ? (
        ...
        <p style={{ color: "rgba(255,255,255,0.5)", fontSize: 14, marginTop: 4 }}>{userEmail}</p>
      </div>

      <div style={{ padding: "0 20px", marginTop: -32 }}>
        {/* Stats card */}
        <div style={{ background: "#18181F", borderRadius: 24, padding: 20, border: `1px solid rgba(212,175,55,0.15)`, boxShadow: "0 8px 30px rgba(0,0,0,0.3)" }}>
          <div style={{ display: "flex", justifyContent: "space-around" }}>
            {[{ label: "Kajian Diikuti", val: stats.kajian }, { label: "Pesanan", val: stats.orders }].map((s, i, arr) => (
              <>
                <div key={s.label} style={{ textAlign: "center" }}>
                  <p style={{ fontSize: 26, fontWeight: 800, color: GOLD }}>{s.val}</p>
                  <p style={{ fontSize: 12, color: "rgba(255,255,255,0.5)", marginTop: 4 }}>{s.label}</p>
                </div>
                {i < arr.length - 1 && <div style={{ width: 1, height: 40, background: "rgba(212,175,55,0.15)", alignSelf: "center" }} />}
              </>
            ))}
          </div>
        </div>
          ...
          {showOrders && (
            <div style={{ display: "flex", flexDirection: "column", gap: 12, marginTop: 4, padding: "0 4px" }}>
              {orders.length > 0 ? orders.map(order => (
                <div key={order.id} style={{ background: "#18181F", borderRadius: 20, padding: 16, border: `1px solid rgba(212,175,55,0.08)` }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                    <div>
                      <p style={{ fontSize: 13, fontWeight: 700, color: "#fff" }}>{order.orderCode}</p>
                      <p style={{ fontSize: 11, color: "rgba(255,255,255,0.4)", marginTop: 2 }}>{fmtDate(order.date)}</p>
                      {order.resi && <p style={{ fontSize: 11, fontWeight: 600, color: GOLD, marginTop: 4 }}>Resi: {order.resi}</p>}
                    </div>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                    {order.items?.map((item: any, i: number) => (
                      <div key={i} style={{ display: "flex", justifyContent: "space-between", fontSize: 12 }}>
                        <span style={{ color: "rgba(255,255,255,0.6)" }}>{item.name} x{item.qty}</span>
                        <span style={{ fontWeight: 600, color: "#fff" }}>{fmt(item.price * item.qty)}</span>
                      </div>
                    ))}
                  </div>
                  <div style={{ display: "flex", justifyContent: "space-between", marginTop: 12, paddingTop: 12, borderTop: "1px dashed rgba(255,255,255,0.06)" }}>
                  ...
                  {order.status?.toUpperCase() === "PENDING" && (
                    <Link href={`/status/${order.orderCode}`} style={{ display: "block", textAlign: "center", marginTop: 12, padding: "10px", borderRadius: 10, background: GOLD, color: "#0A0A0F", fontSize: 12, fontWeight: 700, textDecoration: "none" }}>Lihat Instruksi Bayar</Link>
                  )}
                  {!["PENDING","FAILED"].includes(order.status?.toUpperCase()) && (
                    <div style={{ marginTop: 12, borderTop: "1px dashed rgba(255,255,255,0.05)", paddingTop: 12 }}>
                      {order.rating ? (
                        <div style={{ background: "rgba(212,175,55,0.06)", padding: 12, borderRadius: 12, border: `1px solid rgba(212,175,55,0.12)` }}>
                          <p style={{ fontSize: 11, fontWeight: 700, color: GOLD, display: "flex", alignItems: "center", gap: 4, marginBottom: 6 }}>
                            <Star size={11} fill={GOLD} color={GOLD} /> Testimoni Anda ({order.rating}/5)
                          </p>
                          {order.testimonial && <p style={{ fontSize: 12, color: "rgba(255,255,255,0.6)", fontStyle: "italic" }}>"{order.testimonial}"</p>}
                        </div>
                      ) : (
                        <button onClick={() => { setActiveOrderForReview(order); setReviewRating(5); setReviewText(""); setReviewImages([]); }} style={{ width: "100%", padding: "10px 0", borderRadius: 12, background: "rgba(212,175,55,0.12)", border: `1px solid rgba(212,175,55,0.2)`, color: GOLD, fontSize: 12, fontWeight: 700, cursor: "pointer" }}>
              ...
              )) : <p style={{ textAlign: "center", padding: 20, color: "rgba(255,255,255,0.3)", fontSize: 13 }}>Belum ada riwayat pesanan.</p>}
            </div>
          )}
      ...
      {activeOrderForReview && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center", padding: 20, backdropFilter: "blur(6px)" }}>
          <div style={{ background: "#18181F", borderRadius: 28, width: "100%", maxWidth: 460, padding: 24, border: `1px solid rgba(212,175,55,0.2)`, maxHeight: "90vh", overflowY: "auto", position: "relative" }}>
            <button onClick={() => setActiveOrderForReview(null)} style={{ position: "absolute", top: 20, right: 20, background: "rgba(255,255,255,0.08)", border: "none", width: 32, height: 32, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
              <X size={16} color="rgba(255,255,255,0.6)" />
            </button>
            <h3 style={{ fontSize: 18, fontWeight: 800, color: "#fff", marginBottom: 6 }}>Beri Testimoni</h3>
            <p style={{ fontSize: 12, color: "rgba(255,255,255,0.4)", marginBottom: 20 }}>Pesanan #{activeOrderForReview.orderCode}</p>
            <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
              <div style={{ textAlign: "center" }}>
                <p style={{ fontSize: 12, fontWeight: 700, color: "rgba(255,255,255,0.6)", marginBottom: 10 }}>Bagaimana kualitas produk & pelayanan kami?</p>
                <div style={{ display: "flex", justifyContent: "center", gap: 8 }}>
                  {[1,2,3,4,5].map(s => (
                    <button key={s} onClick={() => setReviewRating(s)} style={{ background: "none", border: "none", cursor: "pointer", padding: 4 }}>
                      <Star size={28} color="#EAB308" fill={s <= reviewRating ? "#EAB308" : "none"} />
                    </button>
                  ))}
                </div>
              </div>
              <div>
                <label style={{ fontSize: 12, fontWeight: 700, color: "rgba(255,255,255,0.6)" }}>Tulis Ulasan</label>
                <textarea placeholder="Ceritakan pengalaman Anda..." value={reviewText} onChange={e => setReviewText(e.target.value)} rows={4}
                  style={{ width: "100%", padding: "12px 14px", borderRadius: 14, border: `1px solid rgba(212,175,55,0.2)`, fontSize: 13, outline: "none", resize: "none", background: "rgba(255,255,255,0.04)", color: "#fff", marginTop: 8 }} />
              </div>
              <div>
                <label style={{ fontSize: 12, fontWeight: 700, color: "rgba(255,255,255,0.6)" }}>Foto Produk</label>
                <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center", marginTop: 8 }}>
                  {reviewImages.map((img, idx) => (
                    <div key={idx} style={{ position: "relative", width: 64, height: 64, borderRadius: 12, overflow: "hidden", border: `1px solid rgba(212,175,55,0.2)` }}>
                  ...
                  {reviewImages.length < 5 && (
                    <label style={{ width: 64, height: 64, borderRadius: 12, border: `2px dashed rgba(212,175,55,0.3)`, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", cursor: "pointer", background: "rgba(212,175,55,0.05)" }}>
              ...
              {uploadingMedia && <p style={{ fontSize: 11, color: GOLD }}>Mengunggah...</p>}
              <div style={{ display: "flex", gap: 12 }}>
                <button onClick={() => setActiveOrderForReview(null)} style={{ flex: 1, padding: "13px 0", borderRadius: 14, border: `1px solid rgba(255,255,255,0.1)`, color: "rgba(255,255,255,0.6)", background: "transparent", fontSize: 14, fontWeight: 700, cursor: "pointer" }}>Batal</button>
                <button onClick={handleSubmitReview} disabled={uploadingMedia || submittingReview || !reviewText.trim()} style={{ flex: 2, padding: "13px 0", borderRadius: 14, border: "none", background: GOLD, color: "#0A0A0F", fontSize: 14, fontWeight: 700, cursor: "pointer", opacity: (uploadingMedia || submittingReview || !reviewText.trim()) ? 0.5 : 1 }}>
