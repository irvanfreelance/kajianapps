...
const EMPTY_CATEGORY: Partial<ProductCategory> = {
  name: "",
  slug: ""
};
...
export default function ProductCategoriesView() {
  ...
  const fetchCategories = async () => {
    setIsLoading(true);
    try {
      const res = await fetch("/api/product-categories/list");
  ...
  const handleDelete = async (id: number, name: string) => {
    if (!window.confirm(`Hapus kategori "${name}"? Kategori tidak dapat dihapus jika masih digunakan oleh produk.`)) return;
    try {
      const res = await fetch("/api/product-categories/delete", {
        method: "POST",
  ...
  return (
    <div style={{ animation: "fadeIn 0.3s ease" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20, flexWrap: "wrap", gap: 12 }}>
        <div>
          <h2 style={{ fontSize: 20, fontWeight: 700, color: "#0F172A", margin: 0 }}>Kelola Kategori Produk</h2>
          <p style={{ fontSize: 13, color: "#64748B", margin: "4px 0 0" }}>
            Atur kategori produk secara dinamis untuk filter pada etalase toko dan form produk.
          </p>
        </div>
      ...
      <div style={{ ...styles.card, padding: 0, overflow: "hidden" }}>
        ...
        {isLoading ? (
          ...
          <div style={{ padding: 48, textAlign: "center", color: "#94A3B8" }}>
            ...
            <p style={{ margin: 0, fontSize: 14 }}>Tidak ditemukan kategori produk.</p>
          </div>
        ) : (
